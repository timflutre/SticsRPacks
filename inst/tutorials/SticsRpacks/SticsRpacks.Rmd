...
The exercises of the tutorial will use the USMs (units of simulation) defined in the "example" workspace of JavaStics.
...
# Simulate with neither Nstress nor water stress
...
p <- plot(sim_ws_ns=sim_ws_ns, sim_ws=sim_ws, sim_ns=sim_ns, sim_without_stress=sim_without_stress,
          var=dyn_var_names)
print(p)
```
...
# Set-up the simulation environment:
wrapper_options = stics_wrapper_options(javastics = javastics_path, workspace = workspace_path,
                                        parallel = TRUE)
...
The function `get_usms_list` from the [SticsRFiles package](https://SticsRPacks.github.io/SticsRFiles/index.html) returns the list of USMs (units of simulation) included in a usms.xml file.
